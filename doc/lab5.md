...
队列的实现可以参考`src/common/list.h`中`Queue`与`queue_`系列函数，这一部分助教已经给出了一个实现，大家可以自行修改或直接使用助教给定的遍历。
...
对于内核的其他模块而言，异步的块设备驱动通常包括三个函数，初始化函数如 `sd_init`、设备读写请求函数如 `sdrw`、设备中断处理函数如 `sd_intr`。让我们从读写函数开始讲起。
...
![sd-read.png](.\images\sd-read.png)
...
**<u>截止时间：2023年11月3日 23:59。</u>逾期提交将扣除部分分数。**
